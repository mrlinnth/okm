...
Two superadmins, authenticated by a single shared admin password — no user accounts. Roughly 60 recipients, each with a public link to their own key/subscription status page — no accounts, no secret phrase.

> Update (2026-08-28): during the build the shared secret became a dedicated
> `adminaccess.password` rather than the Cockpit API token — see the
> "Admin access gate" feature below.
...
**Scope:** A `/manage` login page. A single shared password (`adminaccess.password`, fails closed when empty) unlocks subscription mode; every `/servers*` and `/subscriptions*` route sits behind an `adminauth` + CSRF filter, with unauthenticated GETs redirected to `/manage` and POST/JSON getting a 401. Failed logins are rate-limited per IP. Added during the build — not in the original scope.
**Dependencies:** None
**Priority:** 1
