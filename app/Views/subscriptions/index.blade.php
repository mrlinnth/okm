...
<div x-data="subscriptionLedger()" x-cloak>
    ...
    <div class="mt-6 rounded-box border border-base-300 bg-base-100 p-3 shadow-sm sm:flex sm:items-center sm:gap-3">
        <label class="input input-bordered flex items-center gap-2 sm:flex-1">
        ...
        <div class="mt-2 grid grid-cols-2 gap-2 sm:mt-0 sm:flex">
            <select x-model="filters.status" class="select select-bordered select-sm w-full sm:w-36"><option value="">All statuses</option><option value="active">Active</option><option value="disabled">Disabled</option><option value="expired">Expired</option></select>
            <select x-model="filters.serverId" class="select select-bordered select-sm w-full sm:w-40"><option value="">All servers</option><template x-for="server in servers" :key="server._id"><option :value="server._id" x-text="server.label"></option></template></select>
        </div>
...
function subscriptionLedger() {
    return {
        ...
        async post(url, body = {}) { const response = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Request failed.'); return data; },
