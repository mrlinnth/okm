...
<div x-data="subscriptionLedger()" x-cloak>
        ...
        <table class="table table-zebra"><thead><tr><th>Recipient / key</th><th>Server</th><th>Status</th><th>Expiry</th><th class="text-right">Actions</th></tr></thead>
            <tbody><template x-for="sub in filtered()" :key="sub._id"><tr>
                <td><div x-show="editing.id !== sub._id || editing.field !== 'recipientName'" @click="startEdit(sub, 'recipientName')" class="cursor-pointer font-medium hover:underline decoration-dotted" x-text="sub.recipientName"></div><input x-show="editing.id === sub._id && editing.field === 'recipientName'" x-model="editing.value" @blur="saveEdit(sub)" @keydown.enter.prevent="saveEdit(sub)" class="input input-sm" />
                    <a :href="sub.shareLink" target="_blank" rel="noopener" class="link link-hover block text-xs text-base-content/50" x-text="sub.keyName"></a></td>
    ...
    <div class="modal" :class="{ 'modal-open': editTarget }"><div class="modal-box"><h2 class="text-lg font-semibold">Edit subscription</h2><div class="mt-4 space-y-3"><input x-model="editForm.recipientName" class="input w-full" placeholder="Recipient name" /><input x-model="editForm.expiryDate" type="date" class="input w-full" /></div><p x-show="editError" class="mt-2 text-sm text-error" x-text="editError"></p><div class="modal-action"><button @click="editTarget = null" class="btn btn-ghost">Cancel</button><button @click="saveEditModal()" :disabled="busy" class="btn btn-neutral" x-text="busy ? 'Saving…' : 'Save changes'"></button></div></div><div class="modal-backdrop" @click="editTarget = null"></div></div>
</div>
...
function subscriptionLedger() {
    return {
        ...
        async post(url, body = {}) { const token = document.querySelector('meta[name="X-CSRF-TOKEN"]'); const headers = { 'Content-Type': 'application/json' }; if (token) headers['X-CSRF-TOKEN'] = token.content; const response = await fetch(url, { method: 'POST', headers, body: JSON.stringify(body) }); const data = await response.json().catch(() => null); if (response.status === 401) { window.location.assign(data?.login || '/manage'); return; } if (!response.ok) throw new Error(data?.error || `Request failed (HTTP ${response.status}).`); if (data === null) throw new Error('The server returned an invalid response.'); return data; },
        ...
        openEdit(sub) { this.editTarget = sub; this.editForm = { recipientName: sub.recipientName, expiryDate: sub.expiryDate }; this.editError = ''; },
        async saveEditModal() { if (!this.editTarget) return; this.editError = ''; this.busy = true; try { let updated = this.editTarget; if (this.editForm.recipientName !== this.editTarget.recipientName) { updated = await this.post(`/subscriptions/${this.editTarget._id}`, { recipientName: this.editForm.recipientName }); this.replace(updated); } if (this.editForm.expiryDate !== this.editTarget.expiryDate) { updated = await this.post(`/subscriptions/${this.editTarget._id}/expiry`, { date: this.editForm.expiryDate }); this.replace(updated); } this.editTarget = null; } catch (e) { this.editError = e.message; } finally { this.busy = false; } },
